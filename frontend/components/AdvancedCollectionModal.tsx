...
interface AdvancedCollectionModalProps {
...
export default function AdvancedCollectionModal({ collection, onClose, onSuccess }: AdvancedCollectionModalProps) {
  ...
  useEffect(() => {
    if (isEditing && collection) {
      // Load full collection details including metadata and images
  ...
  const loadCollectionDetails = async (collectionId: number) => {
    try {
      const response = await api.get(`/collections/${collectionId}/details?include_analytics=true`)
          ...
          <div className="flex-1 flex">
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="text-center">
                          <button
                            ...
                            onClick={() => {
                              ...
                              toast('File upload coming soon! Use URL for now.')
                            }}
                          >
                ...
                {activeTab === 'analytics' && (
                  <div className="space-y-6">
                    <h3 className="text-lg font-semibold text-white">Collection Analytics</h3>
                    {collectionDetails ? (
                      <CollectionAnalytics collection={collectionDetails} />
                    ) : (
                      <div className="text-gray-400 text-center py-8">
                        Analytics will be available after the collection is created
                      </div>
                    )}
                  </div>
                )}
