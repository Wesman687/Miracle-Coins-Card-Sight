#!/usr/bin/env python3
"""
Process metafields via API endpoint
"""

import requests
import json

def process_metafields_via_api():
    """Process metafields using API calls"""
    
    base_url = "http://localhost:13000"
    
    print("🔧 Processing Metafields via API")
    print("=" * 50)
        ...
        if response.status_code == 200:
            ...
            for coin in coins:
                ...
                if raw_metafields.get('edges') and not product_metafields and not category_metafields:
                    ...
                    # Update the coin via API (this will fail with 403, but let's try)
                    ...
                    if update_response.status_code == 200:
                        ...
                        print(f"  ⚠️ Update failed for coin {coin_id}: {update_response.status_code}")
                        # Store the processed data for manual update
                        processed_data = {
                            'coin_id': coin_id,
                            'title': title,
                            'product_metafields': new_product_metafields,
                            'category_metafields': new_category_metafields
                        }
...
if __name__ == "__main__":
    success = process_metafields_via_api()
